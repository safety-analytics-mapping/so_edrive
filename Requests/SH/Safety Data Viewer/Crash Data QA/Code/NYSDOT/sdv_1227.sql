...
,coalesce(sum(CASE WHEN accd_typ = '01' or veh_typ = '0' or veh_count>1 THEN num_of_inj END),0) "Unknown"--, advanced_crashes_1227.crashid
FROM advanced_crashes_1227
LEFT JOIN (SELECT distinct crashid
...
GROUP BY CASE WHEN TRAF_CNTL = '01' THEN '001. NONE'
	    ...
	    WHEN TRAF_CNTL = 'XX' THEN '019. NOT ENTERED'
	    WHEN TRAF_CNTL = 'YY' THEN '020. NOT APPLICABLE'
	    WHEN TRAF_CNTL = 'ZZ' or TRAF_CNTL = '00'  THEN '021. UNKNOWN' END--, advanced_crashes_1227.crashid
ORDER BY " ")
...
FROM advanced_crashes_1227
RIGHT JOIN(SELECT distinct advanced_crashes_1227.crashid, num_of_inj
	   FROM advanced_crashes_1227 
	   ...
	   and num_of_inj != 0) t1
on advanced_crashes_1227.crashid = t1.crashid 
GROUP BY CASE WHEN TRAF_CNTL = '01' THEN '001. NONE'
	    WHEN TRAF_CNTL = '02' THEN '002. TRAFFIC SIGNAL'
	    WHEN TRAF_CNTL = '03' THEN '003. STOP SIGN'
	    WHEN TRAF_CNTL = '04' THEN '004. FLASHING LIGHT'
...
where advanced_crashes_1227.crashid = '369231822017'
...
SELECT *
FROM archive."2019_11_13_nysdot_vehicle"
WHERE crashid = '369231822017'


select * from 
advanced_crashes_1227
WHERE crashid = '368319632017'
...
SELECT CASE WHEN TRAF_CNTL = '01' THEN '001. NONE'
	    ...
	    WHEN TRAF_CNTL = 'YY' THEN '020. NOT APPLICABLE'
	    WHEN TRAF_CNTL = 'ZZ' or TRAF_CNTL = '00'  THEN '021. UNKNOWN' END as " "
,CASE WHEN accd_type_int = 1 and nys_v_sex.ped_sex = 'M' and ped_count=1 THEN 'M'
      WHEN accd_type_int = 2 and nys_v_sex.bike_sex = 'M' and bike_count=1 THEN 'M' 
      WHEN accd_type_int = 1 and nys_v_sex.ped_sex = 'F' and ped_count=1 THEN 'F'
      WHEN accd_type_int = 2 and nys_v_sex.bike_sex = 'F' and bike_count=1 THEN 'F'
      WHEN accd_type_int = 3 THEN 'NA'  
      ELSE 'Unknown' END "sex"  
--,accd_type_int      
,advanced_crashes_1227.crashid
...
advanced_crashes_1227 
LEFT JOIN 
archive."2019_11_13_nysdot_vehicle" nys_v
on advanced_crashes_1227.crashid = nys_v.crashid
where advanced_crashes_1227.crashid = '352578282014'
