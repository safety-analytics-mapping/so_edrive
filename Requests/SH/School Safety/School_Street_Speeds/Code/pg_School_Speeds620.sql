...
--select * from speedcameras.raw_school_buildings 

select * from working.school_street_speeds
order by school
where school = "Mary McDowell Friends High School"
...
/*  Messed up Query
        select distinct(a.school), b.schid, a.st_speed, b.street, a.sid, b.School_Geom, b.street_geom
        from(
        select  x.school, x.st_speed, max((y.sid::integer)) SID --For streets with multiple segments, max segment id is selected
        from ((
        select school,max(mx) st_speed 
        from working.school_street_speeds
        group by school
        order by school asc
        ) x --Table selecting streets of max speed grouped by school
        join 
        (select school, schid, street, sid
        from working.school_street_speeds) y -- Table selecting school, sam school id, street, and segment id
        on x.school = y.school
        )
        group by x.school, x.st_speed
        order by x.school asc) a
        join working.school_street_speeds b
        on a.sid = (b.sid::integer) -- Previously generated table containing geometry information
        order by a.school
...
select distinct(a.school), a.schid, a.st_speed, b.street, a.sid, c.school_geom, b.street_geom --Result Table
from(
...
select distinct(samschool_id),site_name, wkb_geometry from speedcameras.raw_school_buildings 
