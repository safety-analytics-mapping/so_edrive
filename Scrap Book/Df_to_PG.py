...
def df_to_sql(df,tbl_name,db): 
    ...
    df.to_sql(name='{}'.format(tbl_name), con= engine, if_exists = 'replace', index=False, schema = 'working')

    return "Complete"


def change_geom(db,tbl_name,geom_name):

    """change_geom(db,tbl_name,geom_name)"""


    db.query("""ALTER TABLE working.{tbl}
                ALTER COLUMN {geom} TYPE Geometry USING {geom}::Geometry;
                
                grant all on working.{tbl} to public;""".format(tbl=tbl_name, geom=geom_name))
